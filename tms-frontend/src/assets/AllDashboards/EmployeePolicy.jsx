  ...
  const fetchEmployeeAcknowledgements = async () => {
    try {
      const res = await axios.get(
        `https:/api-tmsdev-be-ede3ccg8dxd3awbw.southindia-01.azurewebsites.net/policy/employee-ack/${employeeId}`
      );
  ...
  const handleAcknowledge = async (policyId) => {
    try {
      ...
      const res = await axios.post(
        "https:/api-tmsdev-be-ede3ccg8dxd3awbw.southindia-01.azurewebsites.net/policy/acknowledge",
        {
          policyId,
          employeeId,
          employeeName,
