...
function TeamLeaderDashboard({ user }) {
  ...
  const fetchData = async () => {
    try {
      const leavesRes = await axios.get(
        `https:/api-tmsdev-be-ede3ccg8dxd3awbw.southindia-01.azurewebsites.net/leave/team-leader/${user._id}`,
      );
      setLeaves(leavesRes.data.leaves || []);
      
      const regRes = await axios.get(
        `https:/api-tmsdev-be-ede3ccg8dxd3awbw.southindia-01.azurewebsites.net/regularization/team-leader/${user._id}`,
      );
  
      // ✅ Get the requests array from response
      const regularizationData = regRes.data.requests || [];
      ...
      const requestsWithRegularization = regularizationData.filter((req) => {
        return req.regularizationRequest && 
               req.regularizationRequest.status !== null &&
               req.regularizationRequest.status !== undefined;
      });
  
      // ✅ Filter last 3 months
      const lastThreeMonthsData = requestsWithRegularization.filter((req) => {
        const recordDate = new Date(
          req.regularizationRequest?.requestedAt || 
          req.createdAt || 
          req.date ||
          req.regularizationRequest?.createdAt
        );
      ...
      const sortedData = lastThreeMonthsData.sort((a, b) => {
                      ...
                      >
                        {r.regularizationRequest.status === "Pending" ? (
                          <>
                            <button
                              className="btn btn-sm btn-outline-success me-2"
                              onClick={(e) => { //rutuja added e 07-04-26
                                e.stopPropagation();
