...
const isModalOpen = !!selected || showResumeModal;
  ...
  const handleView = async () => {
    ...
    try {
      const res = await fetch(
        `https:/api-tmsdev-be-ede3ccg8dxd3awbw.southindia-01.azurewebsites.net/interviews/manager/${managerId}`,
        {
          headers: {
  ...
  const handleUpdate = async () => {
    try {
      const token = localStorage.getItem("accessToken");
      const res = await fetch(
        `https:/api-tmsdev-be-ede3ccg8dxd3awbw.southindia-01.azurewebsites.net/interviews/managerUpdate/${selected._id}`,
        {
          method: "PUT",
          body: JSON.stringify({
