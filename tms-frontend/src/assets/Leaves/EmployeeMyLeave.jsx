...
function EmployeeMyLeave({ user, refreshKey,fetchNotifications}) {
  ...
  useEffect(() => {
    if (selectedLeave && popupRef.current) {
          ...
          allIds.map(async (id) => {
            try {
              const r = await axios.get(
                `https:/api-tmsdev-be-ede3ccg8dxd3awbw.southindia-01.azurewebsites.net/users/${id}`
              );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`https:/api-tmsdev-be-ede3ccg8dxd3awbw.southindia-01.azurewebsites.net/leave/${id}`, { method: "DELETE" });
      if (!res.ok) {
