...
function TableRowModal({ show, onClose, title, fields }) {
 ...
 return (
    <div
     ...
     style={{
            display: "flex",alignItems: "center",justifyContent: "center",background: "rgba(0,0,0,0.5)",position: "fixed",inset: 0,zIndex: 1050,}}
      onClick={onClose}
    >
      <div
...
function TeamLeadReportTMS({ user }) {
...
}, [selectedDonutStatus, selectedProjectMonth]);
const trapFocus = (e) => {
  if (!popupRef.current) return;
  ...
  const TaskStatusTooltip = ({ active, payload }) => {
    ...
    return (
      <div
        style={{
          background: "#fff",padding: "10px 12px",borderRadius: "8px",boxShadow: "0 4px 12px rgba(0,0,0,0.15)",fontSize: "13px",minWidth: "260px",}}
      >
  ...
  const projectStatusLineData = useMemo(() => {
    ...
    projects.forEach((p) => {
  console.log("STATUS DEBUG:", p.name, p.status);
  ...
  : null;
    ...
    console.log("Projects for graph:", projects);
    return Object.values(monthMap).sort((a, b) => a.sortDate - b.sortDate);
  }, [projects, monthRange]);
  
  useEffect(() => {
  console.log("Graph Data:", projectStatusLineData);
}, [projectStatusLineData]);
...
useEffect(() => {
  const fetchProjects = async () => {
    try {
      ...
      const res = await axios.get(`https:/api-tmsdev-be-ede3ccg8dxd3awbw.southindia-01.azurewebsites.net/${userId}/projects`);

      console.log("Projects API:", res.data);

      const projectList = res.data.projects.map((item) => ({
        ...item.project,
        status: item.project.status || item.status || "In Progress",
        }));
...
async function fetchRequiredDetails() {
  try {
    // Tasks
    const taskResponse = await axios.get(`https:/api-tmsdev-be-ede3ccg8dxd3awbw.southindia-01.azurewebsites.net/tasks/${user._id}`);
    ...
    const projectsResponse = await axios.get(
      `https:/api-tmsdev-be-ede3ccg8dxd3awbw.southindia-01.azurewebsites.net/${user._id}/projects`
    );
    ...
    const projects = projectsResponse.data.projects.map((item) => {
      ...
      return {
        ...project,
        dueDate: project.dueDate || project.startDate,
        deliveryDate: project.deliveryDate || null,

        // ✅ Status calculated from date
        status: calculateStatus(project),
      };
    });
...
const calculateStatus = (project) => {
  ...
  if (diffDays === 0) return "Today is last date";
  return "On Track";
};
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchTeamMembers = async () => {
      ...
      try {
        const response = await fetch(`https:/api-tmsdev-be-ede3ccg8dxd3awbw.southindia-01.azurewebsites.net/${user._id}/members`);
          ...
          >
            ...
            >
              <div
                ...
                style={{ fontSize: "32px",  backgroundColor: "#D1ECF1",  minWidth: "70px",  minHeight: "70px",  color: "#3A5FBE", }}
              >{teamCount}
              </div>
        ...
        <div className="col-12 col-sm-6 col-lg-3">
          ...
          >
            ...
            >
              <div
                className="fw-semibold d-flex align-items-center justify-content-center"
                style={{ fontSize: "32px", backgroundColor: "#FFB3B3", minWidth: "70px", minHeight: "70px", color: "#3A5FBE",}}
              >
                {projectCount}
              </div>
        ...
        <div className="col-12 col-sm-6 col-lg-3">
          ...
          >
            ...
            >
              <div
                className="fw-semibold d-flex align-items-center justify-content-center"
                style={{  fontSize: "32px", backgroundColor: "#FFE493", minWidth: "70px", minHeight: "70px", color: "#3A5FBE", }}
              >
                {delayedCount}
              </div>
        ...
        <div className="col-12 col-sm-6 col-lg-3">
          ...
          >
            ...
            >
              <div
                className="fw-semibold d-flex align-items-center justify-content-center"
                style={{  fontSize: "32px",  backgroundColor: "#D7F5E4",  minWidth: "70px",  minHeight: "70px",  color: "#3A5FBE", }}
              >
                {upcomingCount}
              </div>
            ...
            <div className="table-responsive">
              <table className="table table-hover mb-0">
                <thead style={{ backgroundColor: "#ffffffff" }}>
                  <tr>
                    <th
                      style={{fontWeight: "500", fontSize: "14px", color: "#6c757d", borderBottom: "2px solid #dee2e6", padding: "12px",}}>Task
                    </th>
                    <th
                      style={{fontWeight: "500", fontSize: "14px", color: "#6c757d", borderBottom: "2px solid #dee2e6", padding: "12px",}}>
                      Status
                    </th>
                    <th
                     style={{fontWeight: "500", fontSize: "14px", color: "#6c757d", borderBottom: "2px solid #dee2e6", padding: "12px",}}>
                ...
                <tbody>
                  {employeeTasks.length > 0 ? (
                    paginatedEmployeeTasks.map((task) => (
                      ...
                      >
                        <td
                          style={{padding: "12px",fontSize: "14px",borderBottom: "1px solid #dee2e6",color: "#212529",}}>
                          {task.taskName}
                        </td>
                        <td
                          style={{padding: "12px",fontSize: "14px",borderBottom: "1px solid #dee2e6",color: "#212529",}}>
 ...
 {/* 3️⃣ TABLE / LIST */}
            ...
            <div className="card-body p-0">
              <table className="table table-hover mb-0">
                <thead style={{ backgroundColor: "#fff" }}>
                  <tr>
                    <th style={{fontWeight: 500,fontSize: "14px",padding: "12px", }}>
                      Name
                    </th>
                     <th style={{fontWeight: 500,fontSize: "14px",padding: "12px", }}>
                      Role
          ...
          <div className="card shadow-sm border-0 mb-3">
            <div className="card-body p-0">
              <table className="table table-hover mb-0">
                <thead style={{ backgroundColor: "#ffffffff" }}>
                  <tr>
                    <th style={{fontWeight: "500", fontSize: "14px", color: "#6c757d", borderBottom: "2px solid #dee2e6", padding: "12px", }}>
                      Project Name
                    </th>
                    <th style={{fontWeight: "500", fontSize: "14px", color: "#6c757d", borderBottom: "2px solid #dee2e6", padding: "12px", }}>
                      Status
                ...
                <tbody>
                  {paginatedMyProjects.map((proj) => (
                    ...
                    >
                      <td style={{padding: "12px",fontSize: "14px", borderBottom: "1px solid #dee2e6",color: "#212529", }}>
                        {proj.name}
                      </td>
                       <td style={{padding: "12px",fontSize: "14px", borderBottom: "1px solid #dee2e6",color: "#212529", }}>
              ...
              <ResponsiveContainer width="100%" height={280}>
                <PieChart>
                  ...
                  <text
                    ...
                    style={{  fontSize: "22px",  fontWeight: "700",    fill: "#212529",  }}>
                    {totalTasks}
                  </text>
                ...
                <div className="d-flex align-items-center gap-2">
                  <div className="dropdown">
                    <button
                      ...
                      data-bs-toggle="dropdown"
                      aria-expanded="false"
                      style={{ width: "100%", maxWidth: "120px",  minWidth: "100px",  }}>
                      {monthRange === 0
                        ? "All"
                        : monthRange === 3
                          ? "Last 3 Months"
      ...
      {selectedDonutStatus && (
        <div
          ...
          tabIndex="0"
          onKeyDown={trapFocus}
           style={{    display: "flex",  alignItems: "center",  justifyContent: "center",  background: "rgba(0,0,0,0.5)",  position: "fixed",  inset: 0,  zIndex: 1050, }}
                    ...
                    ([empName, tasks]) => (
                      <div key={empName} className="mb-4">
                        ...
                        <div
                          ...
                          style={{ backgroundColor: "#E8F0FE",color: "#3A5FBE",fontWeight: "600",}}>
...
export default TeamLeadReportTMS;
